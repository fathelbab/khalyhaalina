...
Future<Product> fetchProductById(int productId) async {
  try {
    final response =
        await http.get(apiPath + "/Product/admin/getById?id=$productId");
    if (response.statusCode == 200) {
    } else
      throw response.body;
  } catch (e) {
...
Future<void> signIn(String email, String password) async {
  ...
  try {
    final http.Response response = await http.post(apiPath + "/Account/login",
        headers: {
    ...
    print(resultData['statusCode'].toString());
...
Future<void> register(
  ...
  try {
    final http.Response response =
        await http.post(apiPath + "/Account/register",
            headers: {
              'Content-Type': 'application/json',
            },
            body: json.encode({
